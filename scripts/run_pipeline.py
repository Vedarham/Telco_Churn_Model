...
This script runs the entire pipeline for the Telco Churn Model project. 
It includes data loading, preprocessing, model training, evaluation, and experiment tracking.
...
from src.features.build_feature import build_features     # Feature engineering
...
def main(args):
    ...
    with mlflow.start_run():
        ...
        df_enc = build_features(df, target_col=target, binary_cols=binary_cols, multi_cols=multi_cols)  # Binary encoding + one-hot encoding
        ...
        preprocessing_artifact = {
            ...
            "binary_cols": binary_cols,       # Binary columns list
            "multi_cols": multi_cols          # Multi-category columns list
        }
        ...
        # Save model locally for serving pipeline consistency
        model_path = os.path.join(artifacts_dir, "model.joblib")
        joblib.dump(model, model_path)
        mlflow.log_artifact(model_path)
        print(f"✅ Model saved locally to {model_path} and logged to MLflow")
...
if __name__ == "__main__":
    ...
    p.add_argument("--input", type=str, default="data/Telco_Churn_Dataset.csv",
                   help="path to CSV (e.g., data/Telco_Churn_Dataset.csv)")
